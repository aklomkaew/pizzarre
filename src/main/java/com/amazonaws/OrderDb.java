...
public class OrderDb extends DatabaseTable {
	 ...
	 * 
	 * @param c An Order object representing the order to be added to the Order
	 *          Database
	 * @return True if the Order object can be added, false otherwise If the Order
	 *         object's orderNumber already exists in the Order Database, then the
	 *         Order object cannot be added
	 */
	public static boolean addOrder(Order c) {
		...
		DynamoDBScanExpression scanExpression = new DynamoDBScanExpression().withFilterExpression("OrderNumber = :val1")
	 ...
	 * @return True if the Order object can be deleted, false otherwise If the Order
	 *         object is not found in the Order Database, then the Order object
	 *         cannot be deleted
	 */
	 ...
	 * 
	 * @param obj An Order object representing the object to be updated in the Order
	 *            Database
	 */
	...
	/**
	 * Retrieves all Order object that exists in the Order Database filtered by the
	 * specified orderNumber
	 * 
	 * @param id An integer representing the orderNumber
	 * @return A list of Order representing all filtered Order object in the Order
	 *         Database
	 */
	public static List<Order> retrieveFilteredItem(int id) {
		...
		DynamoDBScanExpression scanExpression = new DynamoDBScanExpression().withFilterExpression("ServerId = :val1")
				.withExpressionAttributeValues(attributeValues);
